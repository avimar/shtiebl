post({
  id: "shammai",
  sub: "Parenting",
  by: "shammais_daughter_in_law",
  hol: "suk",
  age: "2,030 yr",
  votes: "36.4k",
  comments: "3.8k",
  awards: "👶🔨🌿",
  title: "Just had a baby. It's Sukkos. My father-in-law is up on the roof with a hammer so the BABY can be in a sukkah.",
  hook: "Newborns are exempt. Everyone knows newborns are exempt. My father-in-law is Shammai. He took the plaster off the ceiling and put schach over the crib.",
  img: "shammai",
  body: `<p>Mishna: women, slaves and minors are exempt from sukkah, but a child who <b>doesn't need his mother</b> is obligated. Story: Shammai HaZaken's daughter-in-law gave birth, and he <b>removed the plaster from the roof</b> and put schach over the bed, for the baby.</p>`,
  op: `<p>New baby, Baruch Hashem, and it's Sukkos. I'm resting. And from the ceiling I hear chipping.</p>
      <p>It's my father-in-law. He took off the plaster, left the beams, and put schach <b>over the baby's bed</b>. The Gemara says the Mishna tells this story because Shammai was stringent even with the tiniest kids.</p>
      <p>The actual rule, for the rest of you: a child who <b>doesn't need his mother</b> is obligated in sukkah. How do you know your kid is there? Read the comments.</p>`,
  src: "Mishna Sukkah 2:8 · Sukkah 28b",
  thread: [
    {
      by: "Gemara_Sukkah",
      votes: "14k",
      x: `<p>What's a child who doesn't need his mother? The school of R' Yannai: one who goes to the bathroom and his mother doesn't have to wipe him. The other opinion: one who wakes up and <b>doesn't call “Ima! Ima!”</b></p><p>But big kids call Ima too! So: one who wakes up and doesn't keep calling <b>“Ima, Ima”</b> until she comes.</p>`,
      src: "Sukkah 28b",
      replies: [
        {
          by: "tired_abba",
          votes: "8.8k",
          x: `<p>My fourteen-year-old calls “Ima, Ima” until she comes. From the other room. About snacks.</p>`,
          flavor: true,
        },
        {
          by: "Shulchan_Aruch",
          votes: "4.9k",
          x: `<p>That's about <b>age five or six</b>. Obligated in sukkah by the Rabbis, to train him in mitzvos.</p>`,
          src: "Shulchan Aruch OC 640:2",
        },
      ],
    },
    {
      by: "Shammai_HaZaken",
      votes: "11k",
      x: `<p>The beams stayed. Only the plaster came off. Then schach over the bed.</p>`,
      img: "c_roof",
      src: "Mishna Sukkah 2:8",
      replies: [
        {
          by: "roof_neighbor",
          votes: "2.4k",
          x: `<p>We share a roof. I heard all of it. Mazel tov.</p>`,
          flavor: true,
        },
      ],
    },
    {
      by: "Queen_Helene",
      votes: "16k",
      x: `<p>R' Yehuda brings my sukkah as a proof. It was <b>over twenty amos</b> tall, the elders went in and out, and nobody said a word. The Rabbis: she's a woman, she's exempt. R' Yehuda: she had <b>seven sons</b>. And she did everything the way the Sages said.</p>`,
      img: "c_helene",
      src: "Sukkah 2b",
      replies: [
        {
          by: "math_kohen",
          votes: "3.1k",
          x: `<p>The Gemara's math: seven sons, even one a year, and at least one is old enough <b>not to need his mother</b>. So someone in that sukkah was obligated.</p>`,
          src: "Sukkah 2b",
          replies: [
            {
              by: "Queen_Helene",
              votes: "7.5k",
              x: `<p>Correct. The oldest stopped calling “Ima, Ima” years ago. I miss it.</p>`,
              flavor: true,
            },
          ],
        },
      ],
    },
    {
      by: "ima_of_five",
      votes: "1.7k",
      x: `<p>Tip from our house: the kid who stopped calling “Ima” at night is the same kid who wants to sleep in the sukkah. Let him eat there first.</p>`,
      flavor: true,
    },
  ],
});
