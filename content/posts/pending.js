post({
  id: "pending",
  sub: "MildlyInfuriating",
  by: "beinoni_4life",
  hol: "ayt",
  age: "1,750 yr",
  votes: "27.3k",
  comments: "3.1k",
  awards: "⏳📖",
  title: "My verdict has said “PENDING” since Rosh Hashana. The tzaddikim got same-day approval. I've been refreshing for a week.",
  hook: "Three books were opened. Two of them got sealed on the spot. Mine went to review until Yom Kippur, and customer service says the outcome depends on me.",
  img: "pending",
  body: `<p>Rabbi Yochanan: three books are opened on Rosh Hashana. The completely righteous are written and sealed for life <b>immediately</b>. The completely wicked, immediately for death. Everyone in the middle is left <b>hanging until Yom Kippur</b>. Merit, and you're written for life.</p>`,
  op: `<p>Rosh Hashana: three books open. Tzaddikim: written and sealed <b>on the spot</b>. The wicked: same, the other way. Me, a <i>beinoni</i>, exactly in the middle: “your application is under review.” Ten days.</p>
      <p>The Rambam confirms it: a beinoni's verdict stays open until Yom Kippur. If he does teshuva, he's sealed for life. If not, not.</p>
      <p>So I have a pending status, a hard deadline, and the only person who can move my file forward is <b>me</b>. Mildly infuriating. Mostly motivating. Don't tell anyone.</p>`,
  src: "Rosh Hashana 16b · Rambam, Hilchos Teshuva 3:3",
  thread: [
    {
      by: "HaMelech",
      votes: "∞",
      x: `<p class="quote"><span class="he">הֶחָפֹץ אֶחְפֹּץ מוֹת רָשָׁע… הֲלוֹא בְּשׁוּבוֹ מִדְּרָכָיו וְחָיָה</span>: “Do I want the wicked to die? Rather, that he turn back from his ways and live.”</p>`,
      src: "Yechezkel 18:23",
    },
    {
      by: "R_Yitzchak",
      votes: "12k",
      x: `<p>Four things <b>tear up</b> a person's decree: tzedakah, crying out in prayer, a change of name, and a change of deeds. Some add a fifth: a change of place.</p>`,
      src: "Rosh Hashana 16b",
      replies: [
        {
          by: "Yonah_ben_Amitai",
          votes: "3.3k",
          x: `<p>The proof text for “change of deeds” is my city. <i>“And G-d saw their deeds.”</i> I was there. I was not happy about it.</p>`,
          src: "Rosh Hashana 16b, citing Yonah 3:10",
          replies: [
            {
              by: "Melech_Nineveh",
              votes: "8.8k",
              x: `<p>We're a PROOF TEXT 🙌 Framing this.</p>`,
              flavor: true,
            },
          ],
        },
        {
          by: "EY_resident",
          votes: "2.9k",
          x: `<p>“Change of place” is from Avram: <i>“Go for yourself from your land”</i> and then <i>“I will make you a great nation.”</i> The other opinion says that wasn't the move itself, it was the merit of Eretz Yisrael. Either way the answer is the same. Come on over 🌴</p>`,
          img: "c_moving",
          src: "Rosh Hashana 16b · Bereishis 12:1–2",
        },
        {
          by: "speedrunner_bochur",
          votes: "1.1k",
          x: `<p>so I change my name, move apartments, done by Monday?</p>`,
          flavor: true,
          replies: [
            {
              by: "Rambam_1138",
              votes: "5.6k",
              x: `<p>The name change isn't a trick. The penitent changes his name <b>as if to say: “I'm someone else. I'm not the man who did those things.”</b> And he changes <i>all</i> his deeds for the good. The new name only means something if it's true.</p>`,
              src: "Rambam, Hilchos Teshuva 2:4",
            },
          ],
        },
      ],
    },
    {
      by: "Moshe_Rabbeinu",
      votes: "9.4k",
      x: `<p>Rav Nachman bar Yitzchak finds all three books in something I once said: <i>“Erase me, please, from Your book that You have written.”</i> For the record, I said it for the people after the calf. It wasn't a filing strategy.</p>`,
      src: "Rosh Hashana 16b · Shemos 32:32",
    },
    {
      by: "AYT_regular",
      votes: "4.0k",
      x: `<p>Also R' Yitzchak, same page: a person is judged only by his deeds <b>at the time of judgment</b>. From Yishmael: <i>“G-d heard the lad's voice <b>where he is</b>.”</i> Your file shows who you are right now.</p>`,
      src: "Rosh Hashana 16b · Bereishis 21:17",
    },
    {
      by: "still_in_bed",
      votes: "702",
      x: `<p>status: pending (still in bed)</p>`,
      flavor: true,
      replies: [
        {
          by: "AYT_regular",
          votes: "1.8k",
          x: `<p>Selichos is at 6. The window closes Monday night. Get up.</p>`,
          flavor: true,
        },
      ],
    },
  ],
});
