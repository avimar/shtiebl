post({
  id: "yonah",
  sub: "AmITheRasha",
  by: "Yonah_ben_Amitai",
  hol: "yk",
  age: "2,780 yr",
  votes: "88.1k",
  comments: "9.9k",
  awards: "🐋🏆💎",
  verdict: "Asked by ADMIN. No reply from OP.",
  title: "AITR for being mad that my Boss DIDN'T destroy the city I warned?",
  hook: "I tried to quit, got swallowed by a fish, did the job anyway with a FIVE-WORD sermon, and it WORKED. Somehow I'm the bad guy?",
  img: "yonah",
  body: `<p>Long post with a full comment thread. Open it to see how it ends (spoiler: with a question).</p>`,
  op: `<p>Throwaway? No. Everyone knows who I am.</p>
      <p>My Boss told me to go to Nineveh and call out against it. I booked a ship to Tarshish instead (1:3). Storm. The sailors threw me overboard after I told them to (1:12). A fish swallowed me for three days (2:1). Fine. I went.</p>
      <p>My entire sermon: <span class="he">עוֹד אַרְבָּעִים יוֹם וְנִינְוֵה נֶהְפָּכֶת</span>, “Forty more days and Nineveh is overturned” (3:4). <b>Five words.</b> The whole city repented. The king sat in sackcloth. <i>Even the cattle</i> fasted (3:7). And He relented (3:10).</p>
      <p>I told Him: “This is exactly why I ran. I knew You are a gracious and compassionate G-d, slow to anger… Now take my life” (4:2–3). AITR?</p>
      <p><b>UPDATE:</b> I went and sat east of the city to watch. He grew a <i>kikayon</i> plant to shade me, and I was really happy. The next dawn a worm killed it. Then came a hot east wind and sun on my head, and I asked to die again (4:6–8).</p>`,
  src: "Yonah 1–4 · Megillah 31a:11",
  thread: [
    {
      by: "HaMelech",
      votes: "∞",
      x: `<p class="quote">“Are you that deeply angry?”</p>`,
      src: "Yonah 4:4",
      replies: [
        {
          by: "HaMelech",
          votes: "∞",
          x: `<p class="quote">“You cared about the kikayon, which you didn't work for and didn't grow, which appeared overnight and was gone overnight. <b>And shall I not care about Nineveh, the great city, with more than 120,000 people who don't know their right hand from their left, and many animals?</b>”</p>`,
          src: "Yonah 4:10–11",
          replies: [
            {
              by: "Chazzan_Mincha",
              votes: "6.6k",
              x: `<p>And that's the end of the book. It closes on His question. OP never replied. We read this at Mincha on Yom Kippur so that <i>we</i> answer it.</p>`,
              src: "Megillah 31a:11 (Yonah as the YK Mincha haftarah)",
            },
          ],
        },
      ],
    },
    {
      by: "curious_cheder_kid",
      votes: "2.7k",
      x: `<p>why do we read THIS on Yom Kippur though</p>`,
      src: "Megillah 31a:11",
      replies: [
        {
          by: "Mishna_Berura",
          votes: "6.0k",
          x: `<p>Two reasons. It's all about <b>teshuva</b>. And it shows that <b>nobody can run away from Hashem</b>.</p><p>And we don't end on the question: the haftarah closes with <span class="he">מִי אֵל כָּמוֹךָ</span>, “Who is a God like You, who bears sin and passes over transgression.”</p>`,
          src: "Mishna Berura 622:7 · Shulchan Aruch OC 622:2 · Micha 7:18–20",
          replies: [
            {
              by: "Tarshish_Sailor",
              votes: "5.5k",
              x: `<p>can confirm the second one</p>`,
              img: "c_cantrun",
              flavor: true,
            },
          ],
        },
      ],
    },
    {
      by: "Mechilta",
      votes: "8.8k",
      x: `<p>In OP's defense: he ran because gentiles repent quickly, and he didn't want that to make Israel look bad by comparison. “He stood up for the honor of the son, not the honor of the Father.”</p>`,
      src: "Mechilta, Pischa 1",
    },
    {
      by: "Nineveh_Resident",
      votes: "4.1k",
      x: `<p>We didn't just fast. “They turned from the violence in their hands”: if someone had built a stolen beam into his palace, <b>he tore down the palace</b> to give the beam back.</p>`,
      src: "Yonah 3:8 · Taanis 16a:13",
      replies: [
        {
          by: "Melech_Nineveh",
          votes: "31k",
          x: `<p>Look at the effort we went through!! Half my palace is gone. But: <b>FOUND IT</b> 🙌</p>`,
          img: "c_beam",
          flavor: true,
          replies: [
            {
              by: "Yonah_ben_Amitai",
              votes: "-2.4k",
              x: `<p>show-off</p>`,
              flavor: true,
            },
          ],
        },
      ],
    },
    {
      by: "Tarshish_Sailor",
      votes: "2.2k",
      x: `<p>We rowed hard to get back to land first. And we prayed <i>before</i> we threw him in. Meanwhile OP was <b>asleep below deck</b> during the storm.</p>`,
      img: "c_sailors",
      src: "Yonah 1:5, 1:13–14",
    },
    {
      by: "random_cow",
      votes: "19k",
      x: `<p>moo (fasting) (wearing sackcloth) (the king made us)</p>`,
      img: "c_cow",
      src: "Yonah 3:7–8, where even the animals wear sackcloth",
      flavor: true,
    },
  ],
});
