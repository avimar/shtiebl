post({
  id: "shlomo",
  sub: "ThatsInsane",
  by: "Klal_Yisrael",
  hol: "st",
  age: "2,950 yr",
  votes: "61.3k",
  comments: "7.7k",
  awards: "🏛️🎉😱",
  title: "The whole nation ATE on Yom Kippur at a 14-day party. We panicked. Then a Heavenly Voice: “You're all getting Olam HaBa.”",
  hook: "The Beis HaMikdash dedication ran right through Yom Kippur. Nobody fasted. On the eighth day of Sukkos the King sent us home “joyful and glad of heart.” That's the Shemini Atzeres haftara.",
  img: "shlomo",
  body: `<p>R' Yochanan: the year Shlomo dedicated the Mikdash, Yisrael <b>didn't keep Yom Kippur</b>. The seven-day dedication feast covered it. They worried they deserved destruction. <b>A Bas Kol went out: “All of you are destined for the World to Come.”</b></p>`,
  op: `<p>Shlomo HaMelech finished the Beis HaMikdash. The celebration: <i>“seven days and seven days, fourteen days.”</i> Seven for the dedication, seven for Sukkos. The first seven ran <b>through Yom Kippur</b>, and the whole nation feasted.</p>
      <p>Afterward everyone got nervous. Eating on Yom Kippur is <i>kares</i>. Did we just…?</p>
      <p>A Bas Kol: <b>“All of you are destined for the World to Come.”</b></p>
      <p><i>“On the eighth day he sent the people away. They blessed the king and went to their tents, joyful and glad of heart.”</i> The Gemara makes that chapter the haftara of Shemini Atzeres.</p>`,
  src: "Moed Katan 9a:8 · Melachim I 8:65–66 · Megillah 31a:15",
  thread: [
    {
      by: "R_Yochanan",
      votes: "14k",
      x: `<p>R' Parnach said it in my name: that year Yisrael <b>didn't observe Yom Kippur</b>. They were worried. A Bas Kol went out and said: all of you are destined for the World to Come.</p>`,
      src: "Moed Katan 9a:8",
      replies: [
        {
          by: "nervous_yisrael",
          votes: "5.3k",
          x: `<p>Please, nobody try this at home.</p>`,
          flavor: true,
          replies: [
            {
              by: "GabbaiOnDuty",
              votes: "3.2k",
              x: `<p>Seconded. You are not dedicating a Beis HaMikdash this year.</p>`,
              flavor: true,
            },
          ],
        },
      ],
    },
    {
      by: "math_kohen",
      votes: "7.0k",
      x: `<p>How did they figure it was allowed? <b>Kal vachomer.</b> At the Mishkan's dedication, a <i>temporary</i> holiness, the princes' offerings, <i>individual</i> offerings, overrode Shabbos, which is stoning. The Mikdash is <i>permanent</i> holiness with <i>communal</i> offerings. Surely it overrides Yom Kippur, which is only kares.</p>`,
      src: "Moed Katan 9a:9",
      replies: [
        {
          by: "chills",
          votes: "2.6k",
          x: `<p>then why were they worried?</p>`,
          flavor: true,
          replies: [
            {
              by: "math_kohen",
              votes: "4.4k",
              x: `<p>Because the Mishkan overrode Shabbos for <b>offerings to Hashem</b>. Here they overrode Yom Kippur for <b>people eating</b>. So why not bring the offerings and skip the food? The Gemara's answer: <b>there's no complete joy without eating and drinking.</b></p>`,
              src: "Moed Katan 9a:10",
            },
          ],
        },
      ],
    },
    {
      by: "Shlomo_HaMelech",
      votes: "19k",
      x: `<p>Why not just merge the two parties? <b>You don't mix one simcha with another.</b> That's why the pasuk says “seven days <i>and</i> seven days,” and doesn't just say fourteen.</p>`,
      img: "c_twoparties",
      src: "Moed Katan 9a:4–7 · Melachim I 8:65",
      replies: [
        {
          by: "shadchan_2026",
          votes: "4.0k",
          x: `<p>And that's one of the Gemara's reasons you don't make a wedding on Chol HaMoed. One simcha at a time.</p>`,
          src: "Moed Katan 9a:2",
        },
      ],
    },
    {
      by: "HaMelech",
      votes: "∞",
      x: `<p class="quote"><span class="he">הִקְדַּשְׁתִּי אֶת הַבַּיִת הַזֶּה אֲשֶׁר בָּנִתָה</span>: “I have sanctified this House that you have built.”</p>`,
      src: "Melachim I 9:3",
    },
    {
      by: "EY_resident",
      votes: "3.4k",
      x: `<p>Chutz la'aretz still reads <i>“Vayehi kechalos Shlomo”</i> on Shemini Atzeres. Here in Eretz Yisrael it's the same day as Simchas Torah, so we read <i>“Vayehi acharei mos Moshe”</i> instead.</p>`,
      src: "Shulchan Aruch OC 668:2",
      replies: [
        {
          by: "bnei_melachim",
          votes: "2.1k",
          x: `<p>“They blessed the king and went home joyful.” Sounds like <a onclick="go('thread','atzeres')">a Shemini Atzeres I know</a>.</p>`,
          flavor: true,
        },
      ],
    },
  ],
});
