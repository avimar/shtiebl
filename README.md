...
- **Your votes:** ⬆/⬇ on a post is saved in this browser only (`localStorage` key `shtiebl-votes`, `{postId: 1 | -1}`); tapping it again clears it. It colors the arrow and the count. The count itself is content and never changes. Once you've voted, "⬆ Upvoted / ⬇ Downvoted / No vote" chips join the holiday chips (tap again to turn off).
...
- **Feedback:** the `#feedback` box under `#signup` posts to [Web3Forms](https://docs.web3forms.com), which emails each message to us (free plan: 250 a month). The access key in the form is public by design, so no email address or phone number appears in the page. Spam is blocked by hCaptcha, which must be switched on for the form in the [dashboard](https://app.web3forms.com) (otherwise the server doesn't enforce it), plus the hidden `botcheck` honeypot. On submit, the hidden `page` and `viewing` fields are filled with the current link and the view (the post's title in a thread, the query on search), so the email shows what the reader was looking at. To switch the form off, delete or disable the key in the dashboard, or remove the box. (Web3Forms offers reCAPTCHA and Turnstile only on the paid Pro plan.)
